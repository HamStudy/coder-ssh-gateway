...
- NEVER reintroduce hardcoded reserved usernames (`auth@`, `coder@`, `transportUser`, `maintenanceUser`). Removed by design: renewal is inline on any workspace connection; direct-tcpip is available on every authenticated connection; the inner username is agent-determined (Coder hardcodes it — never send `user@target` to the CLI). The two special usernames that DO remain — `login` (enrollment) and `login-admin` (key management) — are CONFIGURABLE, not hardcoded: see the bullet below.
- Configurable special usernames: `enrollment.user` (default `login`) and `key_management.user` (default `login-admin`) live in `config.yaml`, with env-var overrides `CSGW_ENROLLMENT_USER` / `CSGW_KEY_MANAGEMENT_USER` and first-class Helm values `enrollment.{enabled,user}` / `keyManagement.{enabled,user}`. Both must be DNS-label names (charset validated at boot — pre-existing configs with e.g. `Login` now fail validation) and must differ from each other when both features are enabled (collision validation at boot). A special username shadows an equal workspace route — match runs pre-auth, so a workspace literally named `login-admin` is unreachable by username while the feature is enabled. Key-management mode (`internal/keymgmt`, default `login-admin@`): prove an already-enrolled key, run a line-based account-scoped UI on a SINGLE session channel. No Coder API, no credential load, no renewal: the key lookup alone gates the UI. Every other channel type and every forwarding request refused with a logged reason (`internal/server/keymanagement.go`). The session's own key cannot be removed; the session's own key CAN be removed by the typed-`DELETE` account-deletion flow. Audit: `ssh_key_removed`, `account_deleted`.
...
- The inner SSH client currently uses `InsecureIgnoreHostKey()`. This is acceptable by design: the hop is kernel pipes between our own processes (no third party to authenticate — a pipe-level attacker already owns the host and the token in the child's env), and Coder's stdio host key is DETERMINISTIC and publicly derivable (RSA-2048 from FNV-1a over owner/workspace/agent — `SSHKeySeed`/`CoderSigner` in coder/coder), so pinning authenticates nothing an attacker lacks. Optional future nicety: pin the key derived from the PARSED ROUTE as a self-consistency check that catches routing/spawn bugs (wrong-workspace child fails loudly), not as a security control.
