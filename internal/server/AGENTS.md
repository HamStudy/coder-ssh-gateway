...
- Dispatch is mode-driven (the verified-key callback's permissions carry the mode). Workspace mode: session channels bounded CONCURRENTLY by `limits.channels_per_connection` (default 4) per connection, the sshd MaxSessions model — a slot is released when its session channel closes (both the per-connection and per-account slots), so a new session after a closed one succeeds subject to both caps (`channels_per_connection` AND `channels_per_account`; the per-account slot can be consumed by another connection in between); over-cap opens are refused with a logged reason, never silently. Plus concurrent direct-tcpip channels; each channel handled in its own goroutine so multiplexing never blocks (session bridges included). Keymanagement mode: exactly one session channel per connection LIFETIME running the account-scoped key-management UI (`internal/keymgmt`) — an intentional safety property, reconnect to re-open; every other channel type and every forwarding request refused at DEBUG with a logged reason. The post-auth block in `handleConn` constructs the `workspaceContext` and starts the workspace-backed global-requests goroutine **only** inside the `ModeWorkspace` branch — neither runs on keymanagement connections (a `workspaceContext` would let `tcpip-forward` reach the workspace transport, which violates the mode's hard rule).
