...
// parseGlobalFlags peels the persistent flags (--state-dir, --config) off the
// front of args and returns the remaining command line. Global flags must
...
func (c *cli) parseGlobalFlags(args []string) ([]string, error) {
	for len(args) > 0 {
		...
		case len(a) > len("--state-dir=") && a[:len("--state-dir=")] == "--state-dir=":
			name, val = "--state-dir", a[len("--state-dir="):]
			args = args[1:]
		case len(a) > len("--config=") && a[:len("--config=")] == "--config=":
			name, val = "--config", a[len("--config="):]
			args = args[1:]
		default:
			return args, nil
		}
...
Usage:
  coder-ssh-gateway [--state-dir DIR] [--config FILE] <command> [flags]

Commands:
  serve      Run the gateway (config -> store -> SSH server) until signaled
  init       Initialize a state directory (layout, host key, encryption key, starter config)
