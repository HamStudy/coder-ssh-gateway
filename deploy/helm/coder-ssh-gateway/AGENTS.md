...
- `replicaCount` (default 1) may exceed 1: the store coordinates concurrent instances via atomic rename + credential-generation CAS on a shared POSIX volume. Rollouts are `RollingUpdate`. Rate limits are per replica.
