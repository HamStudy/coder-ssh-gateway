...
- `replicaCount` (default 1) may exceed 1: the store coordinates concurrent instances via atomic rename + credential-generation CAS on a shared POSIX volume. `replicaCount > 1` hard-fails template rendering unless `persistence.accessModes` includes `ReadWriteMany` (the PVC must attach to every replica). Strategy is `RollingUpdate` only when replicas > 1; a single replica keeps `Recreate`. Rate limits are per replica.
