...
| Self-key / self-account destructive ops (`login-admin@`) | Account-scoped line-based UI: an authenticated key holder can list and remove their account's OTHER enrolled keys (two-step confirmation), or delete the entire account (every key plus the stored Coder token) behind a typed `DELETE` confirmation. The session's own key cannot be removed via the key-removal flow — refused at selection with the exact message and audited as `ssh_key_removed` failure with `current_session_key`. Workspaces, tunnels, and `exec` are impossible in this mode: exactly one session channel, every other channel type and every forwarding request refused with a logged reason; no Coder API call ever runs. Every removal and every account deletion audited (`ssh_key_removed`, `account_deleted`) with IDs only — no fingerprints, labels, or token bytes. | Blast radius is the holder's own devices and own gateway account: a stolen enrolled key gives the attacker the ability to lock the legitimate user out (by deleting the other keys) or wipe the stored token (forcing re-enrollment with a fresh Coder token). It does NOT reach other accounts, the Coder control plane, the encryption key, or any host key. Disabled `login-admin@` (`key_management.enabled: false`) makes the configured username reject byte-identically to any unknown username. |
